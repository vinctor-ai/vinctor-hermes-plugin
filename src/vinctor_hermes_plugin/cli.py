...
from .boundary import _unmapped_policy, unmapped_policy_warning
...
def _doctor(
    ...
    *,
    ...
    payload: dict[str, Any] = {
        ...
        # PKA-160: the EFFECTIVE policy, not the raw string. An operator who set
        # `Block` and runs `doctor` to check their work was previously shown a
        # clean report that never mentioned the setting they got wrong, while
        # the boundary quietly allowed every unmapped tool. `doctor` exists to
        # answer "what is actually in effect", so this is where it belongs.
        "unmapped_policy": _unmapped_policy(runtime_env),
        "unmapped_policy_warning": unmapped_policy_warning(runtime_env),
    }
    try:
...
def _text(payload: dict[str, Any]) -> str:
    if "config_source" in payload:
        ...
        # PKA-160: the effective unmapped policy rides on every doctor line,
        # valid config or not. It is a property of the ENVIRONMENT, so a broken
        # config must not hide it — those are the runs where an operator most
        # wants to know what the boundary will do.
        policy = f"unmapped_policy: {payload.get('unmapped_policy', 'defer')}"
        warning = payload.get("unmapped_policy_warning")
        if warning:
            policy += f"\nwarning: {warning}"
